...
export function StatsGrid({ data, loading }: StatsGridProps) {
  ...
  const stats = [
    ...
    { title: t.dashboard.totalOffices, value: data?.totalOffices ?? 0, icon: <FiBriefcase />, color: 'from-violet-600 to-violet-700' },
    { title: t.dashboard.totalCars, value: data?.totalCars ?? 0, icon: <FiTruck />, color: 'from-emerald-600 to-emerald-700' },
    { title: t.dashboard.activeCars, value: data?.activeCars ?? 0, icon: <FiCheckCircle />, color: 'from-teal-600 to-teal-700' },
    { title: t.dashboard.pendingRequests, value: data?.pendingRequests ?? 0, icon: <FiClock />, color: 'from-amber-600 to-amber-700' },
    { title: t.dashboard.offersCount, value: data?.offersCount ?? 0, icon: <FiGift />, color: 'from-rose-600 to-rose-700' },
    { title: t.dashboard.favoritesCount, value: data?.favoritesCount ?? 0, icon: <FiHeart />, color: 'from-pink-600 to-pink-700' },
  ];
