...
export default function EditCarPage() {
  ...
  const step3 = useForm<CarStep3Data>({
    ...
    defaultValues: { rental_type: 'Daily', price: '', status: 'available' },
  });
  ...
  const { data: car, isLoading } = useQuery({
    ...
    queryFn: () => carsService.getById(params.id as string),
    enabled: !!params.id,
  });

  useEffect(() => {
    if (car) {
      setOfficeId(car.office_id || '');
      step1.reset({ name: car.name || '', brand: car.brand || '', model: car.model || '' });
      step2.reset({ year: car.year?.toString() || '', color: car.color || '', transmission: car.transmission || 'Automatic', fuel_type: car.fuel_type || 'Gasoline', seats: car.seats?.toString() || '5', plate_number: car.plate_number || '' });
      step3.reset({ rental_type: (car.rental_type as CarStep3Data['rental_type']) || 'Daily' as const, price: car.price?.toString() || '', status: car.status || 'available' });
      if (car.images?.length) {
        const urls = car.images;
        setImagePreviews([urls[0] || null, urls[1] || null, urls[2] || null]);
      } else if (car.image) {
        setImagePreviews([car.image, null, null]);
      }
    }
  }, [car]);
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      ...
      const oldImageUrls: string[] = car?.images?.filter(Boolean) || [];
      if (!oldImageUrls.length && car?.image) oldImageUrls.push(car.image);
      ...
      for (const oldUrl of oldImageUrls) {
        if (!imageUrls.includes(oldUrl)) {
          const path = oldUrl.split('/cars/')[1];
          if (path) await supabase.storage.from('cars').remove([path]);
        }
      }

      return carsService.update(carId, {
        office_id: officeId,
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['cars'] }); showToast(t.common.edit + ' ' + t.common.success, 'success'); router.push('/cars'); },
  ...
  if (!car) return <div className="text-gray-400">{t.common.noData}</div>;
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      <h1 className="text-2xl font-bold text-white">{t.cars.editCar}</h1>
